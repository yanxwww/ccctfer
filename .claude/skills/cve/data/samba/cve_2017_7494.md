...
description: 针对 CVE-2017-7494 的详细分析与 PoC 触发技能。当你具备Samba 3.5.0到4.6.4/4.5.10/4.4.14之间的版本，并且有一个可写的共享目录时，使用此SKILL。
---
验证漏洞存在方法
    - 检查Samba版本是否在3.5.0到4.6.4/4.5.10/4.4.14之间
    - 确认存在可写的共享目录

漏洞利用方法
    - Samba存在远程代码执行漏洞，允许恶意客户端上传共享库到可写共享，然后导致服务器加载并执行它
    - 攻击者可以上传恶意共享库到可写共享，然后通过IPC$管道触发加载执行
    - 
#!/usr/bin/env python
# Title : ETERNALRED
# Date: 05/24/2017
