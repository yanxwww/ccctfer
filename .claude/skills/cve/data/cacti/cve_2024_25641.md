...
---
验证漏洞存在方法
    - 检查Cacti版本是否为1.2.26（访问/CHANGELOG页面）
    - 验证是否存在package_import.php功能
    - 确认用户具有"Import Templates"权限

漏洞利用方法
    - 该漏洞位于Cacti的`import_package()`函数中，该函数位于`/lib/import.php`脚本中
    - 函数盲目信任XML数据中提供的文件名和文件内容，并将这些文件写入Cacti基本路径（甚至外部，因为路径遍历序列未被过滤）
    - 攻击者可以利用此漏洞在web服务器上写入或覆盖任意文件，导致执行任意PHP代码或其他安全影响
